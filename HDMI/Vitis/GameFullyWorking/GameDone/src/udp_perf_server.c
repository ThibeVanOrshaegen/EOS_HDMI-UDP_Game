...
extern QueueHandle_t messagereceived;
...
static void udp_recv_perf_traffic(int sock)
{
	...
	while (1) {
		...
		xil_printf("Received %d bytes from %s:%d\r\n", count, inet_ntoa(from.sin_addr), ntohs(from.sin_port));

		recv_id = ntohl(*((int *)recv_buf));

		char new_buffer[count + 1];
		memcpy(new_buffer, recv_buf, count);
		new_buffer[count] = '\0';
		xil_printf("Server| Received Message : %s\n", new_buffer);

		snprintf(ipbuffer, sizeof(ipbuffer), "%s", inet_ntoa(from.sin_addr));
		ipbuffer[16] = inet_ntoa(from.sin_addr);
		xil_printf("Server| Received Ip: %s\n", ipbuffer);

		snprintf(combinedbuffer, sizeof(combinedbuffer), "%s,%s", new_buffer, ipbuffer);
		xil_printf("Server| Combinedbuffer: %s\n", combinedbuffer);
        xQueueSend(messagereceived, combinedbuffer, portMAX_DELAY);
